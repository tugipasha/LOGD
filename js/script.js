const translations = {
    tr: {
        ...
        vision_label: "Vizyon",
        vision_text: "LOGD, Türkiye'nin lise çağındaki oyun geliştiricileri için ulusal ölçekte tanınan, güvenilir ve sürdürülebilir bir destek ağı olmayı hedefler. Sektör, akademi ve genç yetenek arasında kalıcı bir köprü kurarak Türkiye'yi bölgesinde oyun geliştirme alanında yetenek yetiştiren öncü ülkelerden biri hâline getirmeyi amaçlıyoruz. Her ilde en az bir aktif LOGD topluluğunun bulunduğu, öğrencilerin fikirlerini gerçek ürünlere dönüştürebildiği bir ekosistem inşa ediyoruz. Bu vizyon, kısa vadeli etkinliklerin ötesinde, yıllar içinde büyüyecek bir yetenek havuzu ve kurumsal iş birliği ağı yaratmaya dayanır.",
        mission_label: "Misyon",
        mission_text: "LOGD olarak, öğrencilere öğrenme, üretme ve paylaşma fırsatlarını kesintisiz sunan yapılandırılmış bir topluluk modeli işletiyoruz. Türkiye genelindeki lise topluluklarını destekleyerek, yeni toplulukların kurulmasına rehberlik ediyor ve yerel girişimleri ulusal bir yapı altında birleştiriyoruz. Global Game Jam NEXT gibi uluslararası ölçekli etkinlikler, atölyeler ve mentorluk programları aracılığıyla öğrencileri sektörün gerçek üretim süreçleriyle buluşturuyoruz. Kurumsal ve akademik paydaşlarla kurduğumuz iş birlikleri sayesinde, gençlerin yeteneklerini somut kariyer fırsatlarına dönüştürmelerine katkı sağlıyoruz. Bu misyon, LOGD'yi yalnızca bir topluluk değil, sektörle genç yetenek arasında güvenilir bir köprü kurumu hâline getirir.",
    ...
    en: {
        ...
        vision_label: "Vision",
        vision_text: "LOGD aims to become a nationally recognized, trusted, and sustainable support network for high-school-age game developers across Turkey. By building a lasting bridge between industry, academia, and young talent, we aim to position Turkey as a leading regional hub for game development talent. We are building an ecosystem where every province has at least one active LOGD community, and where students can turn their ideas into real, shippable products. This vision extends beyond short-term events, focused instead on growing a talent pipeline and a corporate collaboration network that compounds year over year.",
        mission_label: "Mission",
        mission_text: "At LOGD, we run a structured community model that gives students uninterrupted access to opportunities for learning, building, and sharing their work. We support high school communities across Turkey, guide the formation of new chapters, and unite local initiatives under one national structure. Through international-scale events like Global Game Jam NEXT, workshops, and mentorship programs, we connect students with the industry's real production workflows. Our partnerships with corporate and academic stakeholders help translate youth talent into concrete career opportunities. This mission positions LOGD not merely as a community, but as a trusted bridge institution between the industry and the next generation of developers.",
...
function changeLanguage(lang) {
    ...
    const footerQuickMenuLinks = document.querySelectorAll('.footer-section:nth-of-type(2) .footer-links li a');
    if (footerQuickMenuLinks[0]) footerQuickMenuLinks[0].textContent = t.nav_home;
    if (footerQuickMenuLinks[1]) footerQuickMenuLinks[1].textContent = t.nav_gallery;
    if (footerQuickMenuLinks[2]) footerQuickMenuLinks[2].textContent = t.nav_about;
    if (footerQuickMenuLinks[3]) footerQuickMenuLinks[3].textContent = t.nav_events;
    if (footerQuickMenuLinks[4]) footerQuickMenuLinks[4].textContent = t.nav_showcase;
    if (footerQuickMenuLinks[5]) footerQuickMenuLinks[5].textContent = t.nav_news;
    if (footerQuickMenuLinks[6]) footerQuickMenuLinks[6].textContent = t.nav_contact;
